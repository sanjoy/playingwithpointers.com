...
I've been going through a set of video lectures from Stanford called
[the theoretical minimum](http://theoreticalminimum.com/courses), and
I figured writing down some study notes will help me better retain
what I'm learning, and once I'm writing these down, why not make them
public.
...
course lectures one through four -- I strongly doubt these notes are
*generally* useful, but it may be helpful for people following that
specific lecture series.

# basics

We start with a system with $$N$$ possible energy states, $$E_i$$
(with $$i$$ ranging from $$1$$ to $$N$$) [^continuous].  These energy
states have been obtained via the specific physics of system we're
studying, and are fixed as far as we're concerned.  The system is in
energy state $$E_i$$ with probability $$P_i$$ and thus $$P_i$$ forms a
probability distribution with $$\sum_1^N P_i = 1$$ and $$P_i >= 0$$.
The entropy of such a system is defined to be $$S = - \sum_1^N P_{i}
\; log P_{i}$$, and is a measure of how "wide" the probability
distribution is.  $$S$$ is a function of both the system and how much
the observer knows about it (I don't yet understand the full
repercussions of this). The average energy is of the system is, by
definition, $$E = \sum_1^N P_{i} \; E_{i}$$.
...
distribution that gives the system the maximum entropy[^entropy].  The
...
n_k\}$$) will the entropy be the highest?".  We also have some
...
is the average energy (per sub-system), and $$\sum_1^k P_i = 1$$.
This (now) purely mathematical optimization problem can be solved
